...
{
    private NMap map;
    public NMap Map { get => map; set => map = value; }
    float _mousePosX;
    float _mousePosY;
...
//        }
        ...
        {
            var newX = Mathf.Clamp(transform.parent.position.x + deltaX * _scrollSpeed, 0, NMap.Width);
            var newY = Mathf.Clamp(transform.parent.position.y + deltaY * _scrollSpeed, 0, NMap.Height);
