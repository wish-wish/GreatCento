...
public class CameraController2 : MonoBehaviour {
	...
	//public CameraPositions2[] cameraPositions;
	...
	private void setPosition() {
			targetPos = cameraPositions[positionIndex].transform.localPosition;
			targetPos.x+=12.96f-0.23f;
			targetPos.y+=1.5f;
			targetPos.z-=4.66f;
			targetRot = cameraPositions[positionIndex].transform.localEulerAngles;
			Index index=cameraPositions[positionIndex].GetComponent<Index>();
			index.pos=targetPos;
			setInfo();
		}
		...
		private void setInfo() {
			...
			descriptionText.text = cameraPositions[positionIndex].name.ToString();
		}
}
