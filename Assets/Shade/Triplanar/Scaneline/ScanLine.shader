...
Shader "Kaima/Depth/ScanLine"
{
	Properties
	{
		...
		_LineWidth("Line Width", Range(0, 0.08)) = 0.005
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float v = saturate(abs(0.05*ftime - linear01Depth) / halfWidth); //线内返回(0, 1)，线外返回1
				fixed4 col = lerp(_LineColor, originColor, v);
				return col;
			}
			ENDCG
		}
